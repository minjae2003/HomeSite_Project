...
<%!
    // HTML 속성 안에 안전하게 넣기 위한 최소한의 이스케이프
    private String escAttr(String s) {
        if (s == null) return "";
        return s.replace("&", "&amp;").replace("\"", "&quot;").replace("<", "&lt;").replace(">", "&gt;");
    }
    // 파일 크기를 KB/MB 단위로 보기 좋게 표시
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .attach-list a { display: inline-flex; align-items: center; gap: 6px; font-size: 14px; color: #2b374e; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 7px 12px; }
        .attach-list a:hover { border-color: #ff5722; color: #ff5722; }
